...
The API client is `src/lib/api/client.ts`. It stores the Sanctum personal access token in browser local storage and sends it as `Authorization: Bearer <token>`. The small `wfp_auth` cookie is only a frontend route-guard hint for Next middleware; it is not an API session cookie.
